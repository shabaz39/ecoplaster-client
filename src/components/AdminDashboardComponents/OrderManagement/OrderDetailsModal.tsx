// src/components/AdminDashboardComponents/OrderManagement/OrderDetailsModal.tsx
import React from 'react';
import { useQuery, gql } from '@apollo/client';
import ShiprocketPanel from './ShipRocketPanel'; // Corrected import name
...
interface OrderDetailsModalProps {
  order: any; // Consider defining a more specific Order type
  onClose: () => void;
  onUpdateStatus: (orderId: string, status: string) => void;
  onUpdateTracking: (orderId: string) => void;
  onAddNote: (orderId: string) => void;
  formatDate: (date: string | number | Date) => string; // Adjust type
  getStatusBadgeClass: (status: string) => string;
  getPaymentStatusBadgeClass: (status: string) => string;
  refetchOrder?: () => void; // Add refetch prop
}
...
const OrderDetailsModal: React.FC<OrderDetailsModalProps> = ({
  ...
  refetchOrder = () => {} // Provide a default empty function
}) => {
  // Get user info
  ...
  return (
    <div
      className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4 overflow-y-auto" // Allow scrolling on the backdrop
      onClick={onClose}
    >
      <div
        className="bg-white rounded-lg shadow-lg w-full max-w-4xl my-8 max-h-[90vh] flex flex-col" // Use flex-col
        onClick={handleModalClick}
      >
        {/* Modal Header */}
        <div className="px-6 py-4 border-b sticky top-0 bg-white z-10 flex justify-between items-center flex-shrink-0">
        ...
        <div className="overflow-y-auto flex-grow">
            ...
            <div className="px-6 py-4 grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <div className="space-y-2">
                  ...
                  {(order.trackingNumber || order.shiprocketAWBCode) && (
                    <p className="text-sm">
                      <span className="font-medium">Tracking Number:</span> {order.trackingNumber || order.shiprocketAWBCode}
                    </p>
                  )}
                  {(order.shippingMethod || order.shiprocketCourier) && (
                    <p className="text-sm">
                      <span className="font-medium">Shipping Method:</span> {order.shippingMethod || order.shiprocketCourier}
                    </p>
                  )}
                ...
                {order.statusHistory?.length > 0 ? (
                  order.statusHistory.map((history: any, index: number) => (
                    <div key={index} className="flex items-center text-sm">
                      <div className="w-32 flex-shrink-0 text-gray-500">{formatDate(history.updatedAt)}</div>
            ...
            <div className="px-6 py-4">
                 <ShiprocketPanel order={order} refetchOrder={refetchOrder} />
            </div>
        </div>

        {/* Action Buttons Footer */}
        <div className="px-6 py-4 border-t flex flex-wrap justify-end gap-3 sticky bottom-0 bg-white flex-shrink-0">
          {/* Status update buttons (conditional rendering based on current status) */}
...
const ProductItem = ({ item }: { item: any }) => {
  ...
  const productName = item.name || product?.name || 'Product not found';
  const productCode = item.code || product?.code || item.productId;
  const productImage = item.image || product?.images?.imageMain;
        ...
        <div className="flex items-center">
          {productImage && (
            <img
              src={productImage}
              alt={productName}
              className="h-10 w-10 mr-3 object-cover rounded border"
            />
          )}
          <div>
